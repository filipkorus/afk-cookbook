...
describe("GET /recipe/ingredient/name", () => {

    test("Unauthorized - should respond with a 401 status code", async () => {
        const response = await supertest(app).get("/recipe/ingredient/ketchup").query(
                {
                page: 1,
                limit: 10,
                }
    ...
    test("Authorized user able to get recipes by ingredient", async () => {
        const response = await supertest(app).get("/recipe/ingredient/eggs").query(
                {
                page: 1,
                limit: 10,
                }
    ...
    test("Authorized user fetches recipes with nonexistent ingredient", async () => {
        const response = await supertest(app).get("/recipe/ingredient/brick").query(
            { page: 1,
              limit: 25,}
        ).set('Authorization', `Bearer ${config.TEST.ACCESS_TOKEN}`)
